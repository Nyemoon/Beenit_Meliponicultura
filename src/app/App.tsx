...
import { Home, BookOpen, ShoppingBag, MessageSquare, User, Bell, Search, Moon, Sun } from "lucide-react";
...
export default function App() {
  ...
  const [theme, setTheme] = useState<Theme>(() => {
    ...
    const stored = window.localStorage.getItem("beenit-theme");
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem("beenit-theme", theme);
  }, [theme]);
  ...
  const tabs = [
    ...
    { id: "knowledge" as Tab, label: "Sobre", icon: BookOpen },
  ...
  if (!isLoggedIn) {
    return <Login theme={theme} onToggleTheme={toggleTheme} onEnter={() => setIsLoggedIn(true)} />;
  }

  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col max-w-md mx-auto border-x border-border/40">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 text-muted-foreground">
            ...
            <button
              ...
              className="inline-flex items-center justify-center w-9 h-9 rounded-2xl bg-muted text-foreground/80 hover:text-foreground hover:bg-muted/80 transition"
            >
        ...
        <div className="grid grid-cols-5 h-[72px] px-2">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                {isActive && <div className="absolute top-0 w-8 h-1 bg-amber-500 rounded-b-full" />}
