    ...
    <div className="max-w-md mx-auto space-y-6 pb-20 bg-background min-h-screen p-4">
      ...
      <section>
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide px-1">
          <Button className="rounded-full bg-secondary hover:bg-emerald-50 hover:text-emerald-900 text-secondary-foreground px-6 h-10 text-xs font-bold">
            Tudo
          </Button>
          {categories.map((cat) => (
            <Button 
              key={cat.name} 
              variant="outline" 
              className="rounded-full border-none bg-card shadow-sm text-muted-foreground px-4 h-10 text-xs font-bold gap-2 whitespace-nowrap hover:bg-emerald-50 hover:text-emerald-900 transition-all"
            >
            ...
            <div className="aspect-[4/5] relative overflow-hidden bg-muted">
              ...
              <div className="absolute top-3 left-3 flex flex-col gap-2">
                {product.status === "Reservado" ? (
                  <Badge className="bg-secondary/90 backdrop-blur-md text-secondary-foreground border-none text-[10px] font-black uppercase">
                    Reservado
                  </Badge>
                ) : (
