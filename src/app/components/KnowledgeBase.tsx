...
const categories = [
  { name: "Todos", icon: BookOpen, count: 133 },
  { name: "Guia de Espécies", icon: Bug, count: 45 },
  { name: "Manejo Técnico", icon: Leaf, count: 38 },
  { name: "Pesquisas", icon: Microscope, count: 62 },
  { name: "Iniciantes", icon: BookOpen, count: 28 }
];
    ...
    <div className="max-w-md mx-auto space-y-8 pb-20 bg-background min-h-screen p-4">
      <section className="space-y-4 pt-2">
        <div className="flex items-center justify-between px-1">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Encontre artigos, guias e estudos para criar com mais segurança.</p>
          </div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide px-1">
          {categories.map((category) => (
            <Button
              ...
              className="rounded-full border-none px-4 h-11 text-xs font-black whitespace-nowrap"
            >
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between px-1 border-l-4 border-amber-500 pl-3">
          ...
          <Button variant="link" className="text-amber-600 font-bold p-0 flex items-center gap-1 text-xs" onClick={() => setSelectedCategory("Todos")}>Ver tudo <ArrowRight className="w-3 h-3" /></Button>
        </div>
        ...
        <div className="space-y-4">
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <h4 className="font-extrabold text-foreground text-sm leading-snug line-clamp-2">{doc.title}</h4>
                  </div>

                  <p className="text-[11px] text-muted-foreground leading-relaxed mb-3 line-clamp-2 font-medium">{doc.description}</p>

                  <div className="flex items-center gap-2 mb-3 flex-wrap">
                    <Badge variant="outline" className="text-[9px] font-black uppercase tracking-wider text-muted-foreground border-border py-0 px-2 h-5">{doc.category}</Badge>
                    <span className="text-[10px] font-bold text-muted-foreground uppercase">{doc.pages} pág.</span>
                    <span className="text-[10px] font-bold text-muted-foreground uppercase">{doc.downloads} downloads</span>
                  </div>
          ...
          {filteredDocuments.length === 0 && (
            <Card className="p-6 border-none shadow-sm bg-card text-center text-muted-foreground">
              Nenhum documento corresponde à sua busca. Tente outro termo.
            </Card>
          )}
        </div>
      </section>
